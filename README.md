...
```
The rviz_click_to_2d node allows the user to specify a target goal robot pose in the Rviz visualization.
...
tf tree command, rosbag, rqt graph to check nodes/topics, rostopic echo topicName, rostopic info topicName, running the binaries for nodes separately with GDB, Rviz to visualize sensor data, TF frames, the path generated by the path-planning algorithm (especially useful was disabling the motor driver node while troubleshooting the path planner). 

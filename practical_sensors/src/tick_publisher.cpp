...
*to be readable for all levels and accompanies the book Practical Robotics in
*C++.
*
...
const int leftReverse =
    13;  // monitor as input that goes low when left motor set to reverse
const int rightReverse =
    20;  // monitor as input that goes low when right motor set to reverse
...
// this is the callback function that runs when a change of state happens on the
// monitored gpio pin
void left_event(int pi, unsigned int gpio, unsigned int edge,
                unsigned int tick) {
  if (gpio_read(pi, leftReverse) ==
      0)  // decrement if motor commanded to reverse
  {
...
// this is the callback function that runs when a change of state happens on the
// monitored gpio pin
void right_event(int pi, unsigned int gpio, unsigned int edge,
                 unsigned int tick) {
